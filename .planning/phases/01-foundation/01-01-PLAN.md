...
must_haves:
  truths:
    ...
    - "Running `docker compose up -d` starts PostgreSQL on port 5432"
  ...
  artifacts:
    ...
    - path: "docker-compose.yml"
      provides: "PostgreSQL local development database"
      contains: "postgres"
...
<objective>
Scaffold the Turborepo monorepo with apps/web (Next.js 15), apps/server (Hono), and packages/shared. Set up PostgreSQL via Docker Compose, Drizzle ORM connection, and the application database schema. After this plan, `npm install && npm run dev` starts both apps and the server can connect to the database.
...
Output: Working monorepo with Hono server, Next.js app, PostgreSQL database, and Drizzle schema.
</objective>
...
<task type="auto">
  <name>Task 2: Docker PostgreSQL + Drizzle DB connection + schema + Hono server entry</name>
  <files>
    docker-compose.yml
    apps/server/src/index.ts
    apps/server/src/db/index.ts
  ...
  <action>
    ...
    3. **apps/server/src/db/schema.ts**: Define user-scoped application tables (Better Auth manages its own tables via its adapter -- these are OUR tables):
       ```typescript
    ...
    4. **apps/server/src/db/index.ts**: Create Drizzle client:
       ```typescript
    ...
    5. **apps/server/src/index.ts**: Minimal Hono server that proves it runs:
       ```typescript
    ...
    6. Copy `.env.example` to `.env` (if it does not exist) so dev server can start.

    7. Start Docker Postgres: `docker compose up -d`

    8. Push schema to database: `cd apps/server && npx drizzle-kit push`

    9. Verify Hono starts: `cd apps/server && npx tsx src/index.ts` (run briefly, ctrl-c after confirming startup log).
  </action>
  <verify>
    - `docker compose ps` shows postgres running on port 5432
    - `cd apps/server && npx drizzle-kit push` succeeds and creates the `user_settings` table
  ...
  <done>
    PostgreSQL is running via Docker. Drizzle schema is pushed to the database with `user_settings` table. Hono server starts on port 3001 and responds to /health. `npm run dev` from root starts both apps via Turborepo.
  </done>
</task>
...
- Docker PostgreSQL runs and accepts connections
- Drizzle schema is pushed with user_settings table
