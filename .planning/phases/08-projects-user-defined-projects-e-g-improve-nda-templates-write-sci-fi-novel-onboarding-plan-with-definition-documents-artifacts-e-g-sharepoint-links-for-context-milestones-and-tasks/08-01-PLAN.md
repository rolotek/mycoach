...
must_haves:
  truths:
    ...
    - "project_tasks table exists with projectId, title, description, status, dueDate, conversationId (optional FK)"
...
5. Add `project_tasks` table:
   ...
   - createdAt, updatedAt: timestamp defaultNow notNull
   - Index: project_tasks_projectId_idx, project_tasks_conversationId_idx on projectId and conversationId
...
8. Add Drizzle relations for projects (user, project_documents, project_links, project_milestones, project_tasks); update user relations and conversation/agentExecution relations to include project where applicable.
</action>
</task>
