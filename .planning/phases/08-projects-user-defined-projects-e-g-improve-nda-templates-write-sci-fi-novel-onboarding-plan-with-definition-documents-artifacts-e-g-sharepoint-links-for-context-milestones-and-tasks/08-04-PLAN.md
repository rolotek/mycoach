...
must_haves:
  truths:
    ...
    - "Project detail page shows definition (name, description, status, due), Documents & links section (attach existing document, add link with url+label), Milestones (each milestone shows its linked tasks; add task directly to a milestone via 'Add task to this milestone' inline form; add/edit/delete milestones), Tasks (tasks without a milestone; add task with optional milestone; per-task milestone selector to move between milestones)"
...
<task type="auto">
  ...
  <files>apps/web/src/app/(app)/projects/[id]/page.tsx</files>
  <action>
1. Milestones section: list from project.get (milestones). Under each milestone, show tasks that have milestoneId equal to that milestone (from project.get tasks). Support adding a task directly to a milestone: "Add task to this milestone" button under each milestone opens an inline form (title, optional description, Add, Cancel); on Add call project.createTask with that milestoneId. Add milestone: form title, optional due date, sortOrder; project.createMilestone. Edit/delete: project.updateMilestone, project.deleteMilestone. Each task row under a milestone has same fields (title, description, status, due, conversation link) plus a milestone dropdown to move task to another milestone or "No milestone"; delete: project.deleteTask.

2. Tasks section: list only tasks where milestoneId is null ("tasks without a milestone"). Add task form: title, description (optional), optional Milestone dropdown (No milestone or pick a project milestone); project.createTask with optional milestoneId. Each task row has a milestone dropdown to assign or change milestone (project.updateTask with milestoneId). Edit/delete as before. Optional: "Link to task thread" — conversationId on task links to /chat/[conversationId].
</action>
</task>
