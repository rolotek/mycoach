...
4. Task procedures:
   - listTasks: query { projectId }; auth; return tasks for project (each task includes milestoneId when set).
   - createTask: mutation { projectId, milestoneId?, title, description?, status?, dueDate? }; auth; if milestoneId provided, verify milestone belongs to same project and user; insert.
   - updateTask: mutation { id, title?, description?, status?, dueDate?, conversationId?, milestoneId? }; auth; verify ownership; if milestoneId provided, verify milestone in project; update.
