...
Purpose: This is the core intelligence layer -- the coaching conversation engine that makes the AI contextual, persistent, and modal. It handles COACH-01 (streaming chat), COACH-02 (references prior context), COACH-04 (auto-detect mode), COACH-05 (manual mode override), and MEM-01 (fact extraction).
...
4. Verify CORS is configured for /api/chat
  ...
  <done>POST /api/chat returns streaming SSE via toUIMessageStreamResponse. System prompt includes coaching persona, mode instructions, RAG context, and user facts. Mode detection works for auto/coaching/task. Chat is mounted in index.ts with proper CORS.</done>
</task>
