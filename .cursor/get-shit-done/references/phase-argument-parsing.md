...
```bash
PHASE_INFO=$(node ./.cursor/get-shit-done/bin/gsd-tools.js find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ./.cursor/get-shit-done/bin/gsd-tools.js roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ./.cursor/get-shit-done/bin/gsd-tools.js find-phase "${PHASE}" --raw)
```
