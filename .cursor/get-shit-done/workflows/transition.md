...
```bash
TRANSITION=$(node ./.cursor/get-shit-done/bin/gsd-tools.js phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node ./.cursor/get-shit-done/bin/gsd-tools.js progress bar --raw)
```
