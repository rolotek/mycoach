...
```bash
INIT=$(node ./.cursor/get-shit-done/bin/gsd-tools.js init plan-phase "$PHASE" --include state,roadmap,requirements,context,research,verification,uat)
```
...
```bash
PHASE_INFO=$(node ./.cursor/get-shit-done/bin/gsd-tools.js roadmap get-phase "${PHASE}")
```
...
```bash
PHASE_DESC=$(node ./.cursor/get-shit-done/bin/gsd-tools.js roadmap get-phase "${PHASE}" | jq -r '.section')
...
STATE_SNAP=$(node ./.cursor/get-shit-done/bin/gsd-tools.js state-snapshot)
